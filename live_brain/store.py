...
class LiveBrainStore:
    def __init__(self, db_path: str, conn_pool=None):
        self.db_path = db_path
        Path(db_path).parent.mkdir(parents=True, exist_ok=True)
        if conn_pool:
            raw_conn = conn_pool.get_connection()
        else:
        ...
        # Initialize managers
        self.schema_mgr = SchemaManager(self.conn)
        self.backup_mgr = BackupManager(self.conn, db_path)
        # MaintenanceManager needs other managers, will be initialized after them
    ...
    def _run_migrations(self) -> None:
        """Delegate to SchemaManager (single source of truth).

        Historically this class had its own copy of migration logic that
        re-raised on failure, which caused the whole memory provider to fail
        initialization if a single migration broke. Now delegates to the
        graceful-degradation version in SchemaManager.
        """
        self.schema_mgr.run_migrations()

    def initialize_schema(self) -> None:
        # The bulk of CREATE TABLE / CREATE INDEX statements that used to be
        # inlined here now live in migrations/000_base_schema.sql. The lines
        # below are still required because they are either (a) non-migration
        # contracts (reality/audit SQL loaded from sibling modules) or
        # (b) defensive additive ALTERs that SQLite's migration engine cannot
        # idiomatically express (adding columns to pre-existing tables).
        self.conn.executescript(SCHEMA_SQL)
        # Run file-based migrations FIRST so base tables (work_items, facts,
        # beliefs, etc.) exist before audit.ensure_schema tries to add its
        ...
        # 000_base_schema.sql (for fresh installs). For existing DBs that predate
        # the migration framework, audit.ensure_schema handles the audit-specific
        # columns, and the base schema CREATE IF NOT EXISTS is idempotent.

        # Indexes that may not exist on very old DBs (safe to re-create).
    ...
    def _expire_self_evolution_rows(
        ...
        *,
        ...
        """Mark ``rows`` as expired in a batched manner.

        Historical implementation issued 3 queries per row (SELECT-before /
        UPDATE / SELECT-after) plus an audit INSERT. For a backlog of 200
        stale proposals this was 600+ round-trips. This version does:

          1. A single ``executemany`` UPDATE for all candidates.
          2. A single IN-clause SELECT for all "after" snapshots.
          3. A single ``executemany`` INSERT for audit_log rows.

        ``record_revision`` still runs per-row because it fans out into
        audit + object_revisions writes, but the UPDATE/SELECT/audit loop is
        now fixed-cost regardless of batch size.
        """
        if not rows:
            if commit:
                self.conn.commit()
            return {'status': 'ok', 'expired': 0, 'proposal_ids': [], 'reasons': {}}

        # Pre-compute per-row payloads without touching the DB.
        prepared: list[dict] = []
        for row in rows:
            proposal_id = str(row['proposal_id'])
            reason = self._self_evolution_expiry_reason(
                row, now=now, stale_hours=stale_hours, e2e_seed_hours=e2e_seed_hours,
            )
            ...
            prepared.append({
                ...
                'reason': reason,
                'before': before,
                'apply_result_json': json.dumps(apply_result, ensure_ascii=False, sort_keys=True),
            })

        # 1) Batch UPDATE: only rows still in 'needs_approval' will be touched.
        self.conn.executemany(
            ...
            """,
            [(now, now, p['apply_result_json'], p['proposal_id']) for p in prepared],
        )

        # 2) Single SELECT for all after-snapshots.
        ids = [p['proposal_id'] for p in prepared]
        placeholder = ','.join('?' for _ in ids)
        after_rows = self.conn.execute(
            f"SELECT * FROM self_evolution_proposals WHERE proposal_id IN ({placeholder})",
            ids,
        ).fetchall()
        after_map = {str(r['proposal_id']): row_to_dict(r) for r in after_rows}

        # 3) Aggregate expired list + audit_log batch.
        expired_ids: list[str] = []
        reasons: dict[str, str] = {}
        audit_rows: list[tuple] = []
        actually_expired: list[dict] = []
        for p in prepared:
            pid = p['proposal_id']
            ...
            reasons[pid] = p['reason']
            actually_expired.append({'prepared': p, 'after': after})
            audit_rows.append((
                stable_id('audit', pid, 'expired', str(int(now))),
                pid,
                p['reason'][:240],
                json.dumps(
                    {
                        'previous_status': p['before'].get('status', ''),
                        'risk_score': p['before'].get('risk_score', 0),
                    },
        ...
        if audit_rows:
            self.conn.executemany(
                ...
                VALUES (?, 'self_evolution_proposal', ?, 'expired', ?, ?, ?)
                """,
                audit_rows,
            )

        # record_revision is intentionally kept per-row because it writes two
        # tables (memory_events + object_revisions) via audit.py helpers. The
        # SELECT-after is already cached in after_map so no extra DB hit here.
        for item in actually_expired:
            record_revision(
                self.conn,
                object_type='self_evolution_proposal',
                object_id=item['prepared']['proposal_id'],
                action='expired',
                reason=item['prepared']['reason'],
                before=item['prepared']['before'],
                after=item['after'],
                created_at=now,
            )
        if commit:
